...
<div class="anjian-main-3-list-box shiminanjian-shijian">
...
<div class="listLine"></div>
<div class="anjian-main-3-list-box shiminanjian-bujian">

</div>
                    
                    </div>
</div>              
<div class="LineMapTitle">近30天高发案件小类预警走势<div class="anjian-btn-box"><div class="anjian-btn-left" @click="shiminXleiClick(0)" :class="{'anjianAc':curId_1==0}">总体</div><div class="anjian-btn-min" @click="shiminXleiClick(1)" :class="{'anjianAc':curId_1==1}">事件</div><div class="anjian-btn-right" @click="shiminXleiClick(2)" :class="{'anjianAc':curId_1==2}">部件</div></div></div>
<div id="shiminSmallLin"></div>        
    ...
    mounted:function(){
        //下钻弹窗
        $('.judge-center').off('click').on('click','.shimin-smallPop',function(){
            var type  = $(this).data('type');
            var code  = $(this).data('id');
            warningPop.detail(type,code)
        })
        this.shiminXleiLin = new cacheData();
        this.shiminXleiLin.setRule({
    ...
    <div class="shimin-smallPop anjian-main-3-list-4" style="top: 14px" data-id="${data.list[i].id}" data-type="${data.list[i].type}"><span class="anjian-card-full-btn card-full-btn"><img src="/dsj_wjjkq_web/public/biz/index/image/home_btn_icon_deep.svg" width="100%"></span></div>
</div>`
                }
                $('.shiminanjian-shijian').html(tpl)
            }
    ...
    methods: {
        ...
        tlvalueClick(){
            var p = pop.view({
                ...
                content: '',
                success: function () {
                    quzhiDetail.init(p.$ow_open_cont,9)
                }
            })
        },
        shiminXleiClick(i){
            let that = this;
            that.curId_1 = i;
            this.shiminXleiLin.getData(i,function(data){
