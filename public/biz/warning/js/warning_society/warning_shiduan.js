let warning_shiduan =  {
    ...
    template:`<div class="judge-center">
                    <div class="judge-center-header">
                        <div class="judge-center-title"><span class="title-name blueShadow-big-title">高发时段预警</span><span class="title-btn" @click="tlvalueClick"><i class="xb_icon info-circle"></i></span></div>
                    ...
                    <div class="shiduan-card-title card-title"><h2 class="card-title-big-2 card-title-big">白天时段</h2>
                    <div class="shiduan-rank-tab-2 rank-tab-2 rank-tab"><a data-type="0" class="rank-tab-item rank-tab-item-1" 
                    @click="shiminDayListClick(0)" :class="{'ac':curId_1==0}">上午</a> <a data-type="1" class="rank-tab-item rank-tab-item-2" 
                    @click="shiminDayListClick(1)" :class="{'ac':curId_1==1}">下午</a></div></div>
...
<div class="shiduan-main-3-list-box anjian-main-3-list-box shehui-shiduan-day">
...
<div class="shiduan-listLine listLine"></div>
<div class="shiduan-main-3-list-box anjian-main-3-list-box shehui-shiduan-night">
...
<div class="LineMapTitle">时段预警近30天走势<div class="anjian-btn-box"><div class="anjian-btn-left" @click="shiminshiduanLinClick(0)" :class="{'anjianAc':curId_3==0}">总体</div><div class="anjian-btn-min" @click="shiminshiduanLinClick(1)" :class="{'anjianAc':curId_3==1}">白天</div><div class="anjian-btn-right" @click="shiminshiduanLinClick(2)" :class="{'anjianAc':curId_3==2}">晚上</div></div></div>
    ...
    mounted:function(){
        $('.judge-center').off('click').on('click','.shehui-shiduanPop',function(){
    ...
    methods: {
        ...
        tlvalueClick(){
            var p = pop.view({
                ...
                content: '',
                success: function () {
                    quzhiDetail.init(p.$ow_open_cont,5)
                }
            })
        },
    ...
    <div class="shehui-shiduanPop anjian-main-3-list-4" style="top: 14px" data-id="${data.list[i].id}" 
    data-type="${data.list[i].type}" data-num="${data.list[i].totalNum}">
    <span class="anjian-card-full-btn card-full-btn"><img src="/dsj_wjjkq_web/public/biz/index/image/home_btn_icon_deep.svg" width="100%"></span></div>
                ...
                $('.shehui-shiduan-day').html(tpl);
                for(let i = 0;i<data.list.length;i++){
                    if(data.list[i].totalNum==0){
                        console.log(i);
                        $('.shehui-shiduan-day').find('.shimin-shiduan-list').eq(i).find('.shehui-shiduanPop').hide()
                    }
                }
            });
    ...
    <div class="anjian-main-3-list-2">${data.list[i].num}</div>
    <div class="anjian-main-3-list-3">${data.list[i].totalNum}</div>
    <div class="shehui-shiduanPop anjian-main-3-list-4" style="top: 14px" 
    data-id="${data.list[i].id}" data-type="${data.list[i].type}" 
    data-num="${data.list[i].totalNum}"><span class="anjian-card-full-btn card-full-btn"><img src="/dsj_wjjkq_web/public/biz/index/image/home_btn_icon_deep.svg" width="100%"></span></div>
</div>`
                }
                $('.shehui-shiduan-night').html(tpl);
                for(let i = 0;i<data.list.length;i++){
                    if(data.list[i].totalNum==0){
                        console.log(i);
                        $('.shehui-shiduan-night').find('.shimin-shiduan-list').eq(i).find('.shehui-shiduanPop').hide()
                    }
                }
            });
        },
